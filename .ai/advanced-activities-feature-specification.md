...
- Backend validates description length ≤ 500 characters using Zod schema
...
- When accordion expands, content displays in the following order:
  ...
  3. Metadata row (duration and transport: "⏱️ 2h • 🚗 15 min")
...
- Timestamps display logic:
  ...
  - If updated_at is different: show "Created: 2026-01-05 14:30 • Updated: 2026-01-05 16:45"
...
- ActivityCard header includes a dropdown menu (⋮ three dots icon) on the right side
...
- Accordion trigger must NOT fire when:
  ...
  - Clicking on dropdown menu (⋮ icon)
...
- AC-001-4: Given the accordion is expanded, when I view the content, then I see timestamps at the top, followed by the description text, then a metadata row showing duration and transport information (e.g., "⏱️ 2h • 🚗 15 min")
...
- AC-001-4b: Given the accordion is expanded for an edited activity, when I view timestamps, then I see "Created: YYYY-MM-DD HH:mm • Updated: YYYY-MM-DD HH:mm"
...
- AC-006-4: Given the activity has no description, when I expand the accordion, then I see "No description added" in the description area and "⏱️ 60 min" in the metadata row
...
- AC-007-1: Given I am viewing an activity, when I click the dropdown menu (⋮ icon) in the ActivityCard header, then I see two options: "Edit" and "Delete"
