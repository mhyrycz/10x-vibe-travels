...
if (response.status >= 500) {
  ...
  throw new OpenRouterApiError(`Server error: ${response.status} ${response.statusText}`, response.status);
}
```
...
try {
  ...
  if (error instanceof z.ZodError) {
    throw new OpenRouterValidationError("Response data does not match expected schema", error);
  }
  throw error;
}
```
...
const jsonSchema = service.convertZodToJsonSchema(testSchema, "test_schema", "A test schema");
...
function buildPrompt(params: CreatePlanDto, userAge?: number, userCountry?: string): string {
